...
export default function ReportWizard({ onClose }) {
  ...
  const handleImageChange = async (e) => {
    ...
    // If item type is Found, perform Image Moderation right away
    if (type === 'found') {
      ...
      try {
        ...
        if (!result.verified) {
          setError(result.reason || 'Image is blocked by moderation rules.');
        } else {
          // Trigger question generation in parallel or proceed
          generateAIQuestions(file);
        }
      } catch (err) {
        console.error(err);
        setError('Image moderation failed. Please try again.');
      } finally {
  ...
  const generateAIQuestions = async (file) => {
    setQuestionsLoading(true);
    try {
      const generated = await aiService.generateQuestions(file);
      if (generated && generated.length === 3) {
        setQuestions(generated);
      }
    } catch (err) {
      console.error('Failed to generate questions, using fallbacks', err);
    } finally {
    ...
    <div>
      ...
      {!imagePreview ? (
        ...
        <div>
          ...
          {/* AI Image verification log */}
          {type === 'found' && moderationLoading && (
            <div className="ai-status-card ai-status-pending">
              <Loader2 className="spinner" size={16} style={{ width: 16, height: 16 }} />
              <span>Gemini AI is analyzing image rules (safe, no human faces)...</span>
            </div>
          )}
          ...
          {type === 'found' && moderationResult && moderationResult.verified && (
            <div className="ai-status-card ai-status-success">
              <ShieldCheck size={16} />
              <span>Gemini Verification: Image is safe and ready. AI questions generated!</span>
            </div>
          )}

          {type === 'found' && moderationResult && !moderationResult.verified && (
            <div className="ai-status-card ai-status-error">
              <AlertCircle size={16} />
              <span>Rejected: {moderationResult.reason || 'Image does not match rules.'}</span>
            </div>
          )}
        </div>
      )}
  ...
  const renderStep4 = () => (
    <div>
      ...
      <p style={{ color: 'var(--light-text)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
        To prevent fraudulent claims, Gemini AI has auto-generated 3 specific questions based on the image you uploaded. You can customize them below.
      </p>
