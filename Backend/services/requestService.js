...
export const requestService = {
  sendClaimRequest: async (item, message = '') => {
    try {
  ...
  getRequests: async (type = 'incoming') => {
    try {
      const user = auth.currentUser;
      if (!user) return [];
  ...
  listenToRequests: (type = 'incoming', callback) => {
    const user = auth.currentUser;
    if (!user) return () => {};
  ...
  getClaimStatus: async (itemId) => {
    try {
      ...
      const userRequest = Object.values(data).find(req => req.claimerId === user.uid);
  ...
  clearRequests: async (requestIds) => {
    try {
      const updates = {};
